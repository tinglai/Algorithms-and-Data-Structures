...
//*************game starts!!!*************************
	...
	compList<list<Zombie*>::iterator> comp(0);
	priority_queue<list<Zombie*>::iterator,vector<list<Zombie*>::iterator>, 
                               compList<list<Zombie*>::iterator>> heap(comp);
...
//cout << endl << "**********total num active zombies to be updated*****" << actZombies.size()<<endl;
		if (!actZombies.empty()){
			(*nextZombie)->attacked(numArrow);
//cout <<endl<< "**********attacked but not dead is: "<<(*nextZombie)->getName()<<endl;
		}
